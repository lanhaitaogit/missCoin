<style scoped src="../assets/css/home.scss" lang="scss"></style>
<style lang="scss">
  html{
    overflow-y: auto;//修复轮播锁滚动条
  }
  .el-carousel__item h3 {
    color: #475669;
    font-size: 14px;
    opacity: 0.75;
    line-height: 150px;
    margin: 0;
  }

  .el-carousel__button {
    height: 6px !important;
  }

  .el-carousel__item:nth-child(2n) {
    background-color: #99a9bf;
  }

  .el-carousel__item:nth-child(2n+1) {
    background-color: #d3dce6;
  }

  .el-carousel--horizontal, .el-carousel__container, .el-carousel {
    height: 670px !important;
    @media only screen and (max-width: 768px) {
      height: 268px !important;
    }
    @media only screen and (max-width: 414px) {
      height: 181px !important;
    }
    @media only screen and (max-width: 375px) {
      height: 205px !important;
    }
  }
</style>
<template>
  <div class="home">
    <div style="height: 80px" id="home"></div>
    <!--pc头部导航-->
    <pc-head @jump="jump" :index="index" ref="head"></pc-head>
    <!--内容部分-->
    <div class="content">
      <div class="block">
        <el-carousel arrow="never">
          <el-carousel-item v-for="(carouse,index) in carouseList" :key="index">
            <el-image
              :src="require('../assets/img/carouse/' + carouse.name)"
              fit="cover"
              class="carouse-img"></el-image>
<!--            <img :src="require('../assets/img/carouse/' + carouse.name)" >-->
          </el-carousel-item>
        </el-carousel>
      </div>
      <div class="preferred">
        <div id="preferred"></div>
        <div class="titles c-c-c">
        </div>
        <div class="preferred-con r-s-a">
          <div class="pre-left c-c-c">
            <div class="l-1 r-l-c">{{$t('preferred.board')}}</div>
            <div class="l-2">{{$t('preferred.intro')}}
            </div>
          </div>
          <div class="pre-right">
            <img src="../assets/img/perfect-img.png">
          </div>
        </div>
      </div>
      <div class="incubation">
        <div id="incubation"></div>
        <div class="content">
          <div class="titles c-c-c">
            <div class="titles-con1">{{$t('incubation.main')}}</div>
            <div class="l-1 r-l-c">{{$t('incubation.title')}}</div>
            <div class="r-l-c">Miss coin
              <!--<span class="details" @click="jump(7)">详情</span>-->
            </div>
            <div class="r-l-c">{{$t('incubation.intro')}}</div>
          </div>
        </div>
        <div class="incubation-con r-s-a warp">
          <div v-for="(icb,index) in icbList" :key="index" class="con-img" @click="jump(icbIndex)">
            <img :src="require('../assets/img/incubator/' + icb.name)">
            <a class="con-title">{{icb.title}}</a>
          </div>
        </div>
      </div>
      <div class="touring">
        <div id="touring"></div>
        <div class="content">
            <div class="l-1 r-l-c">{{$t('touring.title')}}</div>
            <!--<div class="titles-con2">recruitment</div>-->
            <div class="l-2">
              {{$t('touring.intro')}}            </div>
            <div class="enter" @click="jump(6)">{{$t('touring.detail_s')}}</div>
            <div class="exa" @click="jump(7)">
              {{$t('touring.detail_l')}}
            </div>
        </div>
        <div class="pre-right"> </div>
      </div>
      <div class="flow">
        <div id="flow"></div>
        <div class="titles">
          <div class="titles-con1">{{$t('flow.title')}}</div>
        </div>
        <div class="flow-con">
          <div class="flow-con1">
            <div class="titles-con1 l-1">{{$t('flow.title_game')}}</div>
            <div>{{$t('flow.intro_game')}}</div>
            <div class="fbi-content1">
                <div class="fbi-warning1">
                  <div class="logo">{{$t('icon.T')}}</div>
                    <span class="titles">{{$t('flow.title')}}</span>
                    <div class="wire-con1 wire">
                      <div  class="wire-sub1">
                        <span class="wire1" style="margin-left: 18px;">{{$t('flow.game_spending')}}</span>
                        <span class="wire1" style="margin-right: -10px;">{{$t('flow.game_recharge')}}</span>
                      </div>
                    </div>
                  <div class="wire-con2">
                    <div class="wore-sub2 wire">
                      <span class="wire2" style="margin-left: -40px;">{{$t('flow.game_pro')}}</span>
                      <span class="wire2" style="margin-right: -30px;">{{$t('flow.game_def')}}</span>
                    </div>
                    <div class="wore-sub2 wire">
                      <span class="wire2" style="margin-left: -52px;">{{$t('flow.game_platform_recharge')}}</span>
                      <span class="wire2" style="margin-right: -40px;">{{$t('flow.game_fiat')}}</span>
                    </div>
                  </div>
                  <div class="wire-sub3">
                    <div class="wire3 wire">{{$t('flow.game_ex')}}</div>
                  </div>
                  <div class="wire-con3">
                    <div class="wire4 wire" style="margin-left: -40px;">{{$t('flow.game_distribution')}}</div>
                    <div class="wire4 wire" style="margin-right: -40px;">{{$t('flow.game_platform')}}</div>
                  </div>
                </div>
            </div>
          </div>
          <div class="flow-con2">
            <div class="titles-con1 l-1">{{$t('flow.title_payment')}}</div>
            <div>{{$t('flow.intro_payment')}}</div>
            <div class="fbi-content2">
              <div class="fbi-warning2">
                <div class="bank">{{$t('icon.bank')}}</div>
                <div class="logo">{{$t('icon.T')}}</div>
                <div class="wire-sub1">
                  <div class="titles">{{$t('flow.sub_title_payment')}}</div>
                  <div class="wire-con1 wire">
                    <div class="wire-sub2">
                      <span class="wire1" style="margin-left: 18px;">{{$t('flow.card_payment')}}</span>
                      <span class="wire1" style="margin-right: -10px;">{{$t('flow.point_payment')}}</span>
                    </div>
                  </div>
                  <div class="wire-con2 wire">
                    <div class="wire-sub3">
                      <span class="wire2 wire" style="margin-left: 18px;">{{$t('flow.por_co')}}</span>
                      <span class="wire2 wire" style="">{{$t('flow.tour_co')}}</span>
                      <span class="wire2 wire" style="">{{$t('flow.pyramid_co')}}</span>
                    </div>
                  </div>
                  <div class="wire3 wire">{{$t('flow.cloud_payment')}}</div>
                </div>
              </div>
              <div class="list">
                <div><span class="check-item"></span>{{$t('flow.l1')}}</div>
                <div><span class="check-item"></span>{{$t('flow.l2')}}</div>
                <div><span class="check-item"></span>{{$t('flow.l3')}}</div>
                <div><span class="check-item"></span>{{$t('flow.l4')}}</div>
                <div><span class="check-item"></span>{{$t('flow.l5')}}</div>
              </div>
            </div>
            <div class="intro">{{$t('flow.intro_flow')}}</div>

          </div>
          <div class="flow-con3">
            <div class="titles-con1 l-1">{{$t('flow.title_gt')}}</div>
            <div>{{$t('flow.intro_gt')}}</div>
            <div class="fbi-content3">
              <div class="fbi-warning3">
                <div class="logo">{{$t('icon.T')}}</div>
                <div class="wire-sub1">
                  <div class="titles">{{$t('flow.sub_title_gt')}}</div>
                  <div class="wire-con1 wire">
                    <div class="wire-sub2">
                      <span class="wire1" style="margin-left: 18px;">{{$t('flow.detail1_gt')}}</span>
                      <span class="wire1" style="margin-right: -10px;">{{$t('flow.detail2_gt')}}</span>
                    </div>
                  </div>
                  <div class="wire-con2 wire">
                    <div class="wire-sub3">
                      <span class="wire2 wire" style="margin-left: 18px;">{{$t('flow.detail3_gt')}}</span>
                      <span class="wire2 wire" style="">{{$t('flow.detail4_gt')}}</span>
                      <span class="wire2 wire" style="">{{$t('flow.detail5_gt')}}</span>
                    </div>
                  </div>
                  <div class="wire3 wire">{{$t('flow.detail6_gt')}}</div>
                </div>
              </div>
              <div class="list">
                <div><span class="check-item"></span>{{$t('flow.s1')}}</div>
                <div><span class="check-item"></span>{{$t('flow.s2')}}</div>
                <div><span class="check-item"></span>{{$t('flow.s3')}}</div>
                <div><span class="check-item"></span>{{$t('flow.s4')}}</div>
                <div><span class="check-item"></span>{{$t('flow.s5')}}</div>
                <div><span class="check-item"></span>{{$t('flow.s6')}}</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="about-us">
        <div id="about-us"></div>
        <div class="titles c-c-c">
          <div class="titles-con1">{{$t('about.title')}}</div>
        </div>
        <div class="aboutUs-con">{{$t('about.detail')}}</div>
      </div>
      <div class="road">
        <img src="../assets/img/intro/131580458483_.pic_hd.jpg">
      </div>
      <div class="concat" id="concat" style="display: none">
        <div class="split">
          <span>—</span>
          <span>发起机构</span>
          <span>—</span>
        </div>
        <div class="concat-con r-l-c warp">
          <div class="concat-con-img" v-for="(sp,index) in sponsorList" :key="index">
            <img :src="require('../assets/img/sponsor/' + sp.name)">
          </div>
        </div>
      </div>
      <div class="concat-us">
        <div class="titles c-c-c">
          <div class="titles-con1">{{$t('com.title')}}</div>
          <div class="about-comm">{{$t('com.detail')}}</div>
        </div>
        <div class="concat-us-con r-s-a warp">
          <div class="c-s-con c-c-c">
            <div class="concat-img"><img src="../assets/img/concat-us/ho_ic_cont_tele.png"></div>
            <div class="concat-titles">Telegram</div>
            <div class="concat-value">+82-10-7568-9777</div>
          </div>
          <div class="c-s-con c-c-c">
            <div class="concat-img"><img src="../assets/img/concat-us/ho_ic_cont_email.png"></div>
            <div class="concat-titles">Email</div>
            <a class="concat-value" href="mailto:tokencan@tokencan.com">tokencan@tokencan.com</a>
          </div>
          <div class="c-s-con c-c-c">
            <div class="concat-img"><img src="../assets/img/concat-us/ho_ic_cont_tel.png"></div>
            <div class="concat-titles">Tel</div>
            <a class="concat-value" href="tel:+82-10-7568-9777">+82-10-7568-9777</a>
            <a class="concat-value" href="tel:17801076753">+86 17801076753</a>

          </div>
        </div>
      </div>
    </div>
    <pc-foot></pc-foot>
  </div>
</template>
<script>
    import {icbList} from "../assets/js/incubator.js"
    import {sponsorList} from "../assets/js/sponsor.js"
    import {carouseList} from "../assets/js/carousel.js"
    import pcHead from "../components/header";
    import pcFoot from "../components/footer";


    export default {
        name: 'home',
        components: {pcHead, pcFoot},
        data() {
            return {
                icbList: icbList,
                sponsorList: sponsorList,
                carouseList: carouseList,
                index: this.$route.query.type ? this.$route.query.type : 0,
            }
        },
        created() {
            this.index = this.$route.query.type ? this.$route.query.type : 0;
        },
        mounted() {
            this.$refs.head.indexTab=parseInt(this.index);
            setTimeout(() => {
                if (this.$route.query.type) {
                    this.jump(parseInt(this.$route.query.type))
                }
            }, 100)
        },
        methods: {
            //锚点链接
            jump(type) {
                this.index = type;
                switch (type) {
                    case 0:
                        document.getElementById("home").scrollIntoView({block: "start", behavior: "smooth"});
                        break;
                    case 1:
                        document.getElementById("preferred").scrollIntoView({block: "start", behavior: "smooth"});
                        break;
                    case 2 :
                        document.getElementById("touring").scrollIntoView({block: "start", behavior: "smooth"});
                        break;
                    case 3:
                        document.getElementById("incubation").scrollIntoView({block: "start", behavior: "smooth"});
                        break;
                    case 4 :
                        document.getElementById("about-us").scrollIntoView({block: "start", behavior: "smooth"});
                        break;
                    case 8 :
                         document.getElementById("flow").scrollIntoView({block: "start", behavior: "smooth"});
                        break;
                    case 5:
                        this.$router.push('/subProject');
                        break;
                    case 6:
                        this.$router.push('/tour');
                        break;
                    case 7:
                        this.$router.push('/incubator');
                        break;
                }
            },
            // 跳转到百币榜单列表
            goOptimal() {
                this.$router.push('/optimal')

            }
        },
    };

</script>
